...
expresiones}~\cite{togersen:2004}. Dicho problema es un problema clásico en el diseño software.
%=============================================================================%
...
En la siguiente subsección se procederá a detallar de una manera más exhaustiva el problema aquí mencionado.
...
\begin{enumerate}
	...
	\item \imp{ShortEval}: debe evaluar la expresión realizando las operaciones \emph{cortocircuitadas}. Es decir, tan pronto como el valor de un operando determine el resultado de la expresión, se deberá dejar de evaluar el resto de los operandos. Es decir cuando se presenta una multiplicación si el primer operando es 0, se retornará el valor 0 directamente, sin llegar a evaluar el segundo operando.
\end{enumerate}
...
En la siguiente subsección se procederá a detallar como se ha resuelto el problema de las expresiones utilizando el lenguaje orientado a objetos C\#.
...
%=============================================================================%
Para resolver el problema que se presenta, se ha seguido la forma natural de diseñar e implementar las líneas de productos software mediante un lenguaje orientado a objetos. Por un lado se diseña un diagrama UML que soporte los operandos básicos(\emph{Integer, Add y Mult}), tal y como se muestra en la figura ~\ref{back:fig:expr}. A partir de éste, por cada operación: \emph{printInfix}(\emph{print} en formato infijo),\emph{printPostfix} ( \emph{print} en formato posfijo), \emph{printPrefix} (\emph{print} en formato prefijo),\emph{eval}, y \emph{shortEval}(que será una especialización de la propia operación \emph{eval}), se debe realizar un diseño que herede de las clases e interfaces de las que se componía la figura ~\ref{back:fig:expr}. A modo de ejemplo se incluye la figura ~\ref{back:fig:printInfix} que muestra el diseño para la operación \emph{printInfix}.
...
Con el diseño anteriormente mencionado y el objetivo de poder crear diferentes productos a través de la composición de diferentes características, por ejemplo combinar \emph{printInfix} con \emph{eval} para obtener una nueva aplicación que pueda utilizar ambas características, necesitamos hacer uso de la herencia múltiple.
...
No obstante, la solución para trabajar con herencia múltiple en lenguajes orientados a objetos con herencia simple, sería aplicar el \emph{patrón mixin}~\cite{david:1986}. De acuerdo con dicho patrón, que se ilustra en la Figura XX, se utilizan las interfaces para simular la herencia múltiple. Por lo tanto, cada clase a heredar implementará una nueva interfaz que defina los métodos de esta clase, a su vez la nueva clase que se quiere crear para que herede la funcionalidad de sendas clases implementará las dos nuevas interfaces creadas anteriormente, y tendrá una relación de agregación con las clases a heredar.
...
Por lo tanto, algunos autores \cite{laguna:2007} sostienen que, las clases parciales C\# representan una alternativa a la herencia múltiple para manejar variabilidad relacionada programación orientada a características.
\begin{figure}[!t]
\begin{center}
\begin{footnotesize}
\begin{verbatim}
