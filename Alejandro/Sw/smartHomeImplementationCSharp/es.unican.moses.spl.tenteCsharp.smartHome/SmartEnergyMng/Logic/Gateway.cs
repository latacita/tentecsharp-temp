...
namespace SmartHome
{
    ...
    public partial class Gateway : ISubject<IGatewayGUIObserver>
    {
        ...
        public void initSmartEnergyMng(GatewayGUI gGUI, Time time)
        {
            this.gGUI = gGUI;
            this.time = time;
        ...
        public void smartEnergy_checkTime()
        {
            if (statusSmartEnergyMng == true)
            {
                for (int i = 0; i < emptyTime.Count; i = i + 2)
                {
                    if (emptyTime[i] <= time.getTime() && time.getTime() <= emptyTime[i + 1])
                    {
                        if ((emptyTime[i + 1] - time.getTime()) <= 0.60) //20 minutes before             
