...
%%==================================================================%%
Tras explicar a grandes rasgos las transformaciones de modelo a código en la sección \ref{dom:sec:transf}, se procede a comentar la implementación de los generadores de código correspondientes. La figura \ref{dom:fig:templates} muestra la jerarquía de los generadores de código implementados.

\begin{figure}[!tb]
  \center
  \includegraphics[scale=0.80,angle=0]{domainEngineering/images/Templates.eps} \\
  \caption{Jerarquía de los generadores de código implementados en la fase de Ingeniería de Dominio}
  \label{dom:fig:templates}
\end{figure}

El punto de partida es el generador de código llamado \imp{ProjectCreation}, a partir de él se invoca a la plantilla \imp{ClassFilesCreation} que determina si un elemento es:
 ...
 Aunque no aparezca en el diagrama, todos los generadores de código utilizan además el fichero \emph{Operations.eol}, que es un fichero EOL que dispone de operaciones básicas que se utilizan recurrentemente en el resto de generadores, por nombrar algunos ejemplos, dicho fichero contiene funciones del estilo:
 \begin{itemize}
...
\begin{figure}[!tb]
  ...
  \caption{Ventana de selección de modelo}
  \label{dom:fig:selecMod}
\end{figure}
...
\end{figure}
 ...
 \begin{itemize}
   ...
   \item pluginWriteInFile, genera el fichero que contendrá la información para mostrar al usuario al final la ejecución, también implementa funciones para borrar ficheros y/o directorios en el caso de que fuera oportuno.
   \item pluginChooseModel, genera la ventana de selección de modelo para aquellos proyectos que tienen varios (figura \ref{dom:fig:selecMod}).
 \end{itemize}
