...
El punto de partida es el generador de código llamado \imp{ProjectCreation}, encargado de procesar el elemento \emph{modelo}, que constituye la raíz del proyecto, así como los \emph{paquetes} que contiene dicho modelo. Con ello generamos el proyecto \emph{Visual Studio 2010} que constituye la salida del generador, más la estructura de carpetas acorde al modelo de características definido.
 
A partir de él se invoca a la plantilla \imp{ClassFilesCreation} que determina si un clasificador contenido dentro de un paquete es:

\begin{itemize}
   \item Una clase o interfaz, en tal caso llamaría a su vez al generador de código \imp{ClassCreation}.
   \item Una clase con herencia múltiple, en cuyo caso se invoca a \imp{ClassCreationMultipleHierarchyCase}, \imp{ChildClassHierarchyCase} e \imp{InterfaceCreation}.
\end{itemize}
